...
public class ElasticSearchIndexClient implements IndexClient {
	...
	private static TransportClient createClusterTransferClient() {
		...
		// Settings settings =
		// ImmutableSettings.settingsBuilder().put("cluster.name", "es-cluster-"
	...
	private void createIndexIfNeeded() {
		IndicesExistsResponse existsResponse = this.client.admin().indices()
				.prepareExists(INDEX_NAME).execute().actionGet();
		if (!existsResponse.isExists()) {
			CreateIndexRequestBuilder cirb = this.client.admin().indices()
	...
	@Override
	public SearchResultAccumulator queryBackup(String query, String source,
			String type, String job, String username) {
		Map<String, List<String>> filters = createFiltersFor(source, type, job);
		return queryBackup(query, filters, username);
	}

	private Map<String, List<String>> createFiltersFor(String source,
			String type, String job) {
	...
	public SearchResultAccumulator queryBackup(String query,
			Map<String, List<String>> filters, String username) {
	...
	private SearchResponse queryBackup(String query,
		...
		QueryBuilder qBuilder = IndexUtils.buildQuery(this.userId, queryString,
				filters);
		this.logger.debug("#######################################");
		...
		return this.client.prepareSearch(INDEX_NAME).setQuery(qBuilder)
				...
				.addHighlightedField(IndexFields.FIELD_FULLTEXT).setSize(100)
				.execute().actionGet();
	}
	...
	private SearchResponse searchByJobId(long jobId) {
		QueryBuilder qBuilder = QueryBuilders.matchQuery(
				IndexFields.FIELD_JOB_ID, jobId);
		return this.client.prepareSearch(INDEX_NAME).setQuery(qBuilder)
				.execute().actionGet();
	}
	...
	private SearchResponse getFileById(String fileId) {
		...
		QueryBuilder qBuilder = QueryBuilders
				.boolQuery()
				.must(QueryBuilders.matchQuery(IndexFields.FIELD_OWNER_ID,
						owner))
				.must(QueryBuilders.matchQuery(IndexFields.FIELD_FILE_HASH,
						hash))
				.must(QueryBuilders.matchQuery(IndexFields.FIELD_BACKUP_AT,
						timestamp));
	...
	public void deleteRecordsForUser() {
		boolean hasIndex = this.client.admin().indices()
				.exists(new IndicesExistsRequest("indexName")).actionGet()
				.isExists();
		if (hasIndex) {
			this.client
					.prepareDeleteByQuery(INDEX_NAME)
					.setQuery(
							QueryBuilders.matchQuery(
									IndexFields.FIELD_OWNER_ID, this.userId))
					.execute().actionGet();
		}
	}
	...
	public void deleteRecordsForJobAndTimestamp(Long jobId, Long timestamp) {
		...
		this.client.prepareDeleteByQuery(INDEX_NAME).setQuery(qBuilder)
				.execute().actionGet();
	}
	...
	public void index(IndexDocument document) throws IOException {
		...
		XContentBuilder elasticBuilder = new ElasticContentBuilder(document)
				.asElastic();
		this.client.prepareIndex(INDEX_NAME, DOCUMENT_TYPE_BACKUP)

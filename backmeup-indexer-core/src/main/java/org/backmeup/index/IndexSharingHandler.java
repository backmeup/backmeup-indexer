...
/**
 * This class represents the handle for importing and deleting shared index
 * fragments When a backup is shared with another user IndexDocument is stored
 * in a deposit box encrypted with the users public key which gets then updated
...
public class IndexSharingHandler {
	...
	private static final Logger log = LoggerFactory
			.getLogger(IndexSharingHandler.class);

	/**
	 * User A grants permission to user B on a specific indexDocument This is
	 * copied into user B's public drop off area and gets ingested into
	 * elasticsearch when the user logs into the system. The object must have
	 * already been stored in the data sink and is references via UUID Shared
	 * objects will have the same UUID for two different users
	 * 
	 * @param fromUserID
	 * @param withUserID
	 * @param indexFragment
	 ...
	 */
	public static UUID shareIndexFragment(int fromUserID, int withUserID,
			UUID objectID) throws IOException {
		// TODO
		return null;
	}

	public static void revokeIndexFragmentSharing(int fromUserID,
			int withUserID, UUID objectID) throws IOException {
		// TODO
	}
	...
	/**
	 * The ES index gets dropped. The IndexDocuments within the user's fragment
	 * directory + the shared fragments from other users are taken to rebuild
	...
	/**
	 * Imports all user owned index fragments into ES which have not yet been
	 * imported into the index
	 * 
	 * @param userID
	 */
	...
	private static void importIndexFragmentInES(IndexDocument doc, int userID) {
		...
		try {
			Client client = IndexManager.getInstance().getESTransportClient(
					userID);
			index(client, doc);
	...
	private static IndexResponse index(Client client, IndexDocument document)
			throws IOException {
		log.debug("Sending IndexDocument to ES index...");
		XContentBuilder elasticBuilder = new ElasticContentBuilder(document)
				.asElastic();
		IndexResponse response = client
				.prepareIndex(ElasticContentBuilder.INDEX_NAME,
						ElasticContentBuilder.DOCUMENT_TYPE_BACKUP)
				.setSource(elasticBuilder).setRefresh(true).execute()
				.actionGet();

		log.debug("ingested in index: " + response.getIndex() + " type: "

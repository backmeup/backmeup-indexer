...
/**
 * Logic regarding the data container where we get the crypt file from. This
 * uses a real file provider ("data sink") to delegate the real file handling
 * work to.
 * 
...
public class UserDataStorage {
    ...
    /**
     * Init a user specific elasticsearch instance i.e. copies the container
    ...
    private void init(User userID) {
        ...
        try {
            ThemisDataSink.saveIndexTrueCryptContainer(
                    getClass().getClassLoader().getResourceAsStream(CRYPT_TEMPLATE), userID);
        } catch (IOException e) {
    ...
    private File copyTCContainerFileToLocalWorkingDir(File fTCContainerOnDataSink, User userID) throws IOException {
        return copyFileUsingChannel(fTCContainerOnDataSink, new File(UserDataWorkingDir.getDir(userID)
                + "/index/"+CRYPT_FILE));
    }
     ...
     */
    private File copyFileUsingChannel(File source, File dest)
    		throws IOException {
